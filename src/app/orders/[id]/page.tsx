  ...
  if (!order) return (
    <div className="min-h-screen bg-[#fff4e4] text-[#2b160c] flex flex-col">
      ...
      <div className="flex-1 flex items-center justify-center flex-col text-center px-6">
        <h1 className="font-playfair text-3xl font-black text-[#2b160c] mb-2">Order Not Found</h1>
        <p className="text-[#8f6b52]">We couldn't find the order you're looking for.</p>
      </div>
    </div>
  );
  ...
  return (
    <div className="min-h-screen bg-[#fff4e4] text-[#2b160c] flex flex-col">
      ...
      <main className="flex-1 max-w-4xl mx-auto w-full px-6 pt-32 pb-20">
        <div className=" rounded-3xl p-8 border border-[#ead8c1] shadow-sm mb-8">
          <div className="text-center mb-10">
            <h1 className="font-playfair text-3xl font-black text-[#2b160c] mb-2">Track Your Order</h1>
            <p className="text-[#8f6b52] font-medium">Order #{order.orderId}</p>
          </div>
          ...
          {isCancelled ? (
            ...
            <div className="relative mb-16 sm:mb-12 px-4 sm:px-0 mt-6 sm:mt-0">
              ...
              <motion.div
                className="absolute top-1/2 left-4 sm:left-0 h-1.5 bg-[#ef5a24] -translate-y-1/2 rounded-full transition-all duration-1000"
                initial={{ width: 0 }}
                  ...
                  return (
                    <div key={step.status} className="flex flex-col items-center">
                      <motion.div
                        ...
                        className={`w-8 h-8 sm:w-10 sm:h-10 rounded-full flex items-center justify-center z-10 transition-colors shadow-sm ${isActive ? "text-[#2b160c]" : "text-[#8f6b52]"}`}
                      >
                        <Icon className="w-4 h-4 sm:w-5 sm:h-5" />
                      </motion.div>
                      <span className={`text-[10px] sm:text-xs font-black mt-2 sm:mt-3 uppercase tracking-wider text-center absolute top-10 sm:top-12 w-16 sm:w-24 -ml-8 sm:-ml-12 transition-opacity ${isActive ? "text-[#ef5a24]" : "text-[#8f6b52]"} ${isCurrent ? "opacity-100" : "opacity-0 sm:opacity-100"}`}>
                        {step.label}
                      </span>
                    </div>
                  );
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {/* Order Details */}
          <div className=" rounded-3xl p-6 border border-[#ead8c1] shadow-sm">
            <h3 className="text-lg font-black text-[#2b160c] mb-6">Order Summary</h3>
              ...
              {order.items.map((item, i) => (
                <div key={i} className="flex gap-4 items-center">
                  ...
                  <div className="flex-1">
                    <p className="font-bold text-[#2b160c] text-sm">{item.productId?.name || "Item"}</p>
                    <p className="text-xs text-[#8f6b52]">Qty: {item.quantity}</p>
                  </div>
                  <p className="font-black text-[#2b160c]">₹{(item.price * item.quantity).toFixed(2)}</p>
                </div>
              ))}
            </div>
            <div className="border-t border-[#ead8c1] pt-4 flex justify-between items-center font-black text-lg">
              <span>Total</span>
              <span className="text-[#ef5a24]">₹{order.total?.toFixed(2)}</span>
            </div>
          </div>
          ...
          <div className="space-y-6">
            <div className=" rounded-3xl p-6 border border-[#ead8c1] shadow-sm">
              <h3 className="text-sm font-black text-[#8f6b52] uppercase tracking-widest mb-4">Delivering From</h3>
              <p className="font-black text-[#2b160c] text-lg mb-1">{order.branchId?.name}</p>
              <p className="text-sm text-[#8f6b52] flex items-start gap-2 mb-3">
                <MapPin className="w-4 h-4 mt-0.5 shrink-0" />
                {order.branchId?.address?.street}, {order.branchId?.address?.city}
              </p>
              <p className="text-sm text-[#8f6b52] flex items-center gap-2">
            ...
            <div className=" rounded-3xl p-6 border border-[#ead8c1] shadow-sm">
              <h3 className="text-sm font-black text-[#8f6b52] uppercase tracking-widest mb-4">Delivery To</h3>
              <p className="text-sm font-bold text-[#2b160c] flex items-start gap-2">
                <MapPin className="w-4 h-4 mt-0.5 shrink-0 text-[#ef5a24]" />
